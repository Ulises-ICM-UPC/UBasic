#
# Mon Jun 27 12:03:33 2022, extract from Ulises by Gonzalo Simarro
#
import copy
import cv2
import datetime
...
def AllVariables2MainSet(allVariables, nc, nr, options={}): # 202109141500
    ...
    return mainSet
def AllVariables2SubsetVariables(dataBasic, allVariables, subsetVariablesKeys, options={}): # 202109251523 (read on 20220625)
    ''' comments:
...
def CR2UaVa(mainSet, cs, rs): # 202109101200
    ...
    return uas, vas
def CR2XY(dataPdfTxt, cs, rs, options={}): # 202109101200 avoid its use -> PlanCR2XY
    ''' comments:
...
def CURU2CDRD(mainSet, cUs, rUs): # 202109101200
    ...
    return cDs, rDs
def CURU2CDRDForParabolicSquaredDistortion(cUs, rUs, oca, ora, k1asa2): # 202206261205
    ''' comments:
...
def ClipWithSign(xs, x0, x1): # 202109101200
    ...
    return xs
def Cloud2Rectangle(xs, ys, options={}): # 202109161400 avoid its use -> see geom2D
    ''' comments:
...
def IntersectionOfTwoLines(line0, line1, options={}): # 202205260841
    ''' comments:
...
def LoadDataPdfTxt(options={}): # 202109101200
    ...
    '''
    def Length2NOfPixels(length, ppm): # 202205131731
        ''' comments:
        ...
        return nOfPixels
    def NOfPixels2Length(nOfPixels, ppm): # 202205131731
        ''' comments:
...
def NonlinearManualCalibration(dataBasic, dataForCal, subsetVariablesKeys, subCsetVariablesDictionary, options={}): # 202109010000
    ...
    .- input subsetVariablesKeys is a string-list
    .- input subCsetVariablesDictionary is a dictionary (given)
...
def NormalizeALine(line, options={}): # 202206201459 (last read on 202206201459)
    ''' comments:
    ...
    return line
def PathImgOrImg2Img(img): # 202205140937
    ''' comments:
    .- input img is a cv2-image or a string
    .- output img is a cv2-image
    '''
    try:
        int(img[img.shape[0]-1, img.shape[1]-1, 0] - 1)
    except:
        img = cv2.imread(img)
    int(img[img.shape[0]-1, img.shape[1]-1, 0] - 1)
    return img
def PerturbateScaledVariables(dataBasic, scaledVariables, variablesKeys, options={}): # 202109241340
    ...
    scaledVariables = VariablesScaling(dataBasic, variables, variablesKeys, 'scale')
    return scaledVariables
def PlanviewPrecomputations(mainSets, dataPdfTxt, z): # 202205140802
    ''' comments:
    ...
    .- input dataPdfTxt is a dictionary (including at least 'nc', 'nr', 'cs', 'rs', 'xs' and 'ys')
        .- dataPdfTxt es herencia de otra forma de trabajar
    ...
    for camera in sorted(mainSets.keys()):
        ...
        ws = CRWithinImage2NormalizedLengthsAndAreas(ncCamera, nrCamera, csCamera, rsCamera)[0] # len(plwPossInCamera)
        csIA, rsIA, wsA1 = CR2CRIntegerAroundAndWeights(csCamera, rsCamera) # all len(plwPossInCamera) x 4
        plwPC['cameras'].append(camera)
        plwPC[camera] = {'planviewPositions':plwPossInCamera, 'ws':ws, 'csIA':csIA, 'rsIA':rsIA, 'wsA1':wsA1}
    return plwPC
def PlotMainSet(img, mainSet, cs, rs, xs, ys, zs, chs, rhs, pathImgOut): # 202111171719
    ...
    return None
def PointInALineClosestToAPoint(line, x, y): # 202205260853 (last read and graphical check at 20220620)
    ''' comments:
    .- input line is a dictionary (including at least 'lx', 'ly' and 'lt')
...
def Random(value0, value1, options={}): # 202109131700
    ...
    return randomValues
def ReadAFirstSeed(dataBasic, dataForCal, subsetVariablesKeys, subCsetVariablesDictionary, mainSetSeeds): # 202109241556 (read on 20220625)
    ''' comments:
    .- input dataBasic and dataForCal are dictionaries
    .- input subsetVariablesKeys is a string-list (keys of the selected variables to be optimized)
    .- input subCsetVariablesDictionary is a dictionary (keys and values of the selected variables that are given)
    ...
    for mainSetSeed in mainSetSeeds:
        ...
        optionsTMP = {'nc':mainSetSeed['nc'], 'nr':mainSetSeed['nr']}
        ...
        if np.max(np.abs(allVariables - mainSetSeed['allVariables'])) > 1.e-6: # IMP* WATCH OUT!!!
...
def ReadCdhTxt(pathCdhTxt, options={}): # 202110051054
    ...
    return chs, rhs
def ReadMainSetFromCalTxt(pathCalTxt, options={}): # 202111171714 (read on 20220625)
    ''' comments:
    .- input pathCalTxt is a string
    .- output mainSet is a dictionary
    '''
    ...
    optionsTMP = {item:options[item] for item in ['orderOfTheHorizonPolynomial', 'radiusOfEarth']}
...
def WriteDataPdfTxt(data): # 202109141700
    ...
    fileout.close()
    return None
def XY2CR(dataPdfTxt, xs, ys, options={}): # 202109101200 avoid its use -> PlanXY2CR
    ''' comments:
    .- input dataPdfTxt is a dictionary (including at least 'ppm', 'angle', 'xUpperLeft', 'yUpperLeft', 'nc' and 'nr')
